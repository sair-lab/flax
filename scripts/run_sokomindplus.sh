#!/bin/bash
export PYTHONPATH=$(pwd):$PYTHONPATH
DOMAIN_NAME="SokomindPlus"
CMPL_RULES="config/sokomindplus_complementary_rules.json"
RELX_RULES="config/sokomindplus_relaxation_rules_1.json"

# PLANNER_TYPE="pure"
# PLANNER_TYPE="ploi"
# PLANNER_TYPE="cmpl"
# PLANNER_TYPE="relx"
PLANNER_TYPE="flax"

TRAIN_TIMEOUT=1200
NUM_TEST_PROBLEMS=100
TEST_TIMEOUT=60

if [ -L "pddlgym/pddl/sokomindplus_test" ]; then
    unlink "pddlgym/pddl/sokomindplus_test"
elif [ -d "pddlgym/pddl/sokomindplus_test" ]; then
    rm -rf "pddlgym/pddl/sokomindplus_test"
fi

# ln -s $(pwd)/pddl_files/problems/sokomindplus_problems/pddl_test_15x15 $(pwd)/pddlgym/pddl/sokomindplus_test   # fd 10-30s
# ln -s $(pwd)/pddl_files/problems/sokomindplus_problems/pddl_test_18x18 $(pwd)/pddlgym/pddl/sokomindplus_test   # fd 40-120s
ln -s $(pwd)/pddl_files/problems/sokomindplus_problems/pddl_test_20x20 $(pwd)/pddlgym/pddl/sokomindplus_test   # fd 60-300s


if [ "$PLANNER_TYPE" = "pure" ]
then
    cmd_str="python -u src/main.py --domain_name $DOMAIN_NAME --test_planner_name fd-lama-first --guider_name no-guidance --num_seeds 1 --num_test_problems $NUM_TEST_PROBLEMS --planner_type $PLANNER_TYPE --test_timeout $TEST_TIMEOUT"
else
    cmd_str="python -u src/main.py --domain_name $DOMAIN_NAME --train_planner_name fd-opt-lmcut --test_planner_name fd-lama-first --guider_name gnn-bce-10 --num_seeds 10 --num_train_problems 200 --num_test_problems $NUM_TEST_PROBLEMS --planner_type $PLANNER_TYPE --train_timeout $TRAIN_TIMEOUT --test_timeout $TEST_TIMEOUT --num_epochs 301 --cmpl_rules $CMPL_RULES --relx_rules $RELX_RULES"
fi

echo "Running Python command:"
echo $cmd_str
eval $cmd_str
