...
class Grid:
    ...
    def __init__(self, width: int, height: int):
        ...
        self.grid: list[WorldObj | None | list[WorldObj]] = [None] * (width * height)

    def __contains__(self, key: Any) -> bool:
        if isinstance(key, WorldObj):
            for e in self.grid:
                if isinstance(e, WorldObj):
                    if e is key:
                        return True
    ...
    def set(self, i: int, j: int, v: WorldObj | None | list[WorldObj]):
        assert (
            0 <= i < self.width
    ...
    def get(self, i: int, j: int) -> WorldObj | None | list[WorldObj]:
    ...
    def process_vis(self, agent_pos: tuple[int, int]) -> np.ndarray:
        ...
        for j in reversed(range(0, self.height)):
            for i in range(0, self.width - 1):
                ...
                cell = self.get(i, j)
                if cell:
                    if isinstance(cell, list) and not cell[-1].see_behind():
                        continue
                    elif isinstance(cell, WorldObj) and not cell.see_behind():
                        continue
